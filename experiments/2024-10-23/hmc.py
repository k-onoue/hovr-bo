...
def bayesian_model(X, y, model_settings):
    ...
    prior_std = model_settings["prior_std"]  # 分散パラメータ
    ...
    for i in range(len(layer_sizes) - 1):
        w = pyro.sample(f"weight_{i}", dist.Normal(0, prior_std).expand([layer_sizes[i+1], layer_sizes[i]]).to_event(2))
        b = pyro.sample(f"bias_{i}", dist.Normal(0, prior_std).expand([layer_sizes[i+1]]).to_event(1))
        weights.append(w)
        biases.append(b)
...
# プロット
def plot_results(X_train, y_train, X_test, mean_preds, std_preds, n_sigma=2, save_path=None):
...
if __name__ == "__main__":
    ...
    for n_sigma in [i * 1e-1 for i in range(2, 10)]:
        ...
        model_settings = {
            ...
            "prior_std": n_sigma  # 事前分布の標準偏差を指定可能
        }
        ...
        # 結果の可視化
        # ここの n_sigma は，可視化のために事後分布の共分散の何σまでを表示するかを指定する
        save_path = os.path.join(LOG_DIR, f"hmc_prior_{n_sigma}.png")
        plot_results(X_train, y_train, X_test, mean_preds, std_preds, n_sigma=2, save_path=save_path)